...
# tensorboard --logdir=runs/train_sam_vit_b_512_logs
...
train_root = os.path.join(BASE_DIR, "data", "augmented_training")
val_root = os.path.join(BASE_DIR, "data", "validation_only_wobble_surface")
batch_size = 8
num_epochs = 30
lr = 1e-5
wd = 1e-4
save_path = os.path.join(BASE_DIR, "checkpoints", "sam_vit_b_01ec64.pth")
device = torch.device("cuda")

log_dir = os.path.join(BASE_DIR, "runs", "train_sam_vit_b_512_logs")
...
class SketchMaskDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_image_processed = self.sam_model.preprocess(input_image_torch.unsqueeze(0)).squeeze(0)  # [3, H, W]
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(train_loader, desc=f"Epoch {epoch+1} [Training]"):
        ...
        gt_mask = batch['mask'].to(device)            # [B, 1, H, W]
        ...
        for i in range(input_image.shape[0]):
            ...
            mask = sam_model.postprocess_masks(
                low_res_masks[i:i+1], input_sz, orig_size
            ).to(device)
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc=f"Epoch {epoch+1} [Validation]"):
            ...
            upscaled_mask = sam_model.postprocess_masks(
                low_res_masks, input_sz, orig_size
            ).to(device)
